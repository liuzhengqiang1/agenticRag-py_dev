...
- **语义缓存**：Redis缓存用户query，向量相似走缓存，不调用LLM。针对否定词敏感度不足会导致缓存污染，在向量相似度基础上，增加否定词检测作为硬性过滤条件。
- **混合检索**：ES 向量检索（kNN）+ BM25 全文检索 + Reranker 重排
- **多轮对话**：Redis 会话存储，支持上下文连续对话（原生redisSaver默认不会做记忆轮数限制，也不会设置 TTL） TODO Redis（短期） + 向量数据库（长期） + 数据库（历史对话+审计） + 记忆压缩（每10轮调用LLM总结） + Memory Service 抽象层（将前面提到的逻辑抽取出来，形成独立的服务，便于多agent共享）
- **流式输出**：SSE 实时响应
